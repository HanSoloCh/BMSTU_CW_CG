...
class CarcasModel : public AbstractModel {
   public:
    ...
    std::array<QVector3D, 3> GetTriangleNormals(
        const std::array<int, 3> &triangle) const;
    std::array<Point, 3> GetTrianglePoints(
        const std::array<int, 3> &triangle) const noexcept;

    std::array<QVector2D, 3> GetTriangleUV(
        const std::array<int, 3> &triangle) const noexcept;
...
CarcasModel GenerateShape(double radius, int slices, int stacks,
                          const QColor &color, const Point &center);
