...
BaseDrawVisitor::BaseDrawVisitor(QPainter *painter)
    : painter_(painter), color_(painter->pen().color()) {}

DrawVisitor::DrawVisitor(QPainter *painter, QSize canvas_size,
                         const AbstractStrategyProjection *projection,
                         QVector<std::shared_ptr<AbstractLight>> light)
    : BaseDrawVisitor(painter),
      canvas_size_(canvas_size),
      z_buffer_(canvas_size_.height(),
                QVector<double>(canvas_size_.width(),
...
QColor IntensityColor(const QColor &color, int intensity) {
    ...
    QColor newColor((color.red() * intensity) / 255,
...
void DrawVisitor::Visit(const Triangle &triangle) {
    ...
    std::transform(points.begin(), points.end(), points.begin(),
                   [&](const Point &p) { return ProjectPoint(p); });
...
void DrawVisitor::DrawTriangle(const Triangle &triangle) {
    auto [bbox_min_x, bbox_min_y, bbox_max_x, bbox_max_y] =
        CalculateBoundingBox(triangle.GetPoints());
    for (int x = bbox_min_x; x <= bbox_max_x; ++x) {
        for (int y = bbox_min_y; y <= bbox_max_y; ++y) {
            BarycentricCoords barycentric_coords;
            if (CalculateBarycentricCoords(triangle.GetPoints(), x, y,
                                           barycentric_coords) &&
                IsInsideTriangle(barycentric_coords)) {
                double z = InterpolateValue<double>(
...
void DrawVisitor::DrawTrianglePixel(
    ...
    QVector3D normal_in_point = InterpolateValue<QVector3D>(
        {normals[0], normals[1], normals[2]}, barycentric_coords);
...
std::tuple<int, int, int, int> DrawVisitor::CalculateBoundingBox(
    ...
    bbox_max_y =
        std::min(bbox_max_y, static_cast<int>(z_buffer_[0].size()) - 1);
    return {bbox_min_x, bbox_min_y, bbox_max_x, bbox_max_y};
}

bool DrawVisitor::CalculateBarycentricCoords(
    const std::array<Point, 3> &pts, int x, int y,
    BarycentricCoords &barycentric_coords) const {
    double denom = (pts[1].y() - pts[2].y()) * (pts[0].x() - pts[2].x()) +
                   (pts[2].x() - pts[1].x()) * (pts[0].y() - pts[2].y());
    if (std::abs(denom) < 1e-9) return false;

    barycentric_coords.alpha = ((pts[1].y() - pts[2].y()) * (x - pts[2].x()) +
                                (pts[2].x() - pts[1].x()) * (y - pts[2].y())) /
                               denom;
    barycentric_coords.beta = ((pts[2].y() - pts[0].y()) * (x - pts[2].x()) +
                               (pts[0].x() - pts[2].x()) * (y - pts[2].y())) /
                              denom;
    barycentric_coords.gamma =
        1.0 - barycentric_coords.alpha - barycentric_coords.beta;
    return true;
}

bool DrawVisitor::IsInsideTriangle(
    const BarycentricCoords &barycentric_coords) const {
    return (barycentric_coords.alpha >= 0 && barycentric_coords.beta >= 0 &&
            barycentric_coords.gamma >= 0);
}
...
int DrawVisitor::CalculateIntensity(const Point &point,
                                    const QVector3D &normal) const {
    double intensity = 0;
    intensity = std::accumulate(
        light_.begin(), light_.end(), 0.0,
...
DrawMappedVisitor::DrawMappedVisitor(
    QPainter *painter, QSize canvas_size,
    const AbstractStrategyProjection *projection,
    QVector<std::shared_ptr<AbstractLight>> light, const QImage &normal_map)
    : DrawVisitor(painter, canvas_size, projection, light),
      normal_map_(normal_map) {}

DrawMappedVisitor::DrawMappedVisitor(const DrawVisitor &draw_visitor,
                                     const QImage &normal_map)
    : DrawVisitor(draw_visitor), normal_map_(normal_map) {}

void DrawMappedVisitor::Visit(const Point &point) { DrawVisitor::Visit(point); }
...
void DrawMappedVisitor::DrawTrianglePixel(
    const QPoint &point, const Triangle &triangle,
    const BarycentricCoords &barycentric_coords) {
    std::array<QVector3D, 3> normals = triangle.GetNormals();
    QVector3D normal_in_point = InterpolateValue<QVector3D>(
    ...
    QVector2D uv_point = InterpolateValue<QVector2D>(
        {uv_in_point[0], uv_in_point[1], uv_in_point[2]}, barycentric_coords);
    normal_in_point += GetNormalInPoint(uv_point);

    int intesity =
        CalculateIntensity(Point(point), normal_in_point.normalized());
...
QVector3D DrawMappedVisitor::GetNormalInPoint(const QVector2D &uv) const {
    int x =
        static_cast<int>(uv.x() * normal_map_.width()) % normal_map_.width();
    int y =
        static_cast<int>(uv.y() * normal_map_.height()) % normal_map_.height();
...
DrawTextureVisitor::DrawTextureVisitor(
    QPainter *painter, QSize canvas_size,
    const AbstractStrategyProjection *projection,
    QVector<std::shared_ptr<AbstractLight>> light, const QImage &texture)
    : DrawVisitor(painter, canvas_size, projection, light), texutre_(texture) {}

DrawTextureVisitor::DrawTextureVisitor(const DrawVisitor &draw_visitor,
                                       const QImage &texutre)
    : DrawVisitor(draw_visitor), texutre_(texutre) {}
...
void DrawTextureVisitor::DrawTrianglePixel(
    const QPoint &point, const Triangle &triangle,
    const BarycentricCoords &barycentric_coords) {
    std::array<QVector3D, 3> normals = triangle.GetNormals();
    QVector3D normal_in_point = InterpolateValue<QVector3D>(
